...
import enhanceCommunicationCommentsTab from '../enhanceCommunicationCommentsTab';
...
it('passes all props added when rendered through of the given Component.', () => {
  ...
  const TestComponent = enhanceCommunicationCommentsTab('div');
...
it('adds the comment data to the props of the given Component.', () => {
  const TestComponent = enhanceCommunicationCommentsTab('div');
  const component = renderer.create(
    <Provider store={makeMockStore()}>
      <TestComponent />
